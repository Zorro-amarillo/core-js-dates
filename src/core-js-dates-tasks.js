...
function getNextFriday(date) {
  ...
  const daysUntilFriday = currentDay < 5 ? 5 - currentDay : 5 - currentDay + 7;
  const fridayDate = currentDate.getDate() + daysUntilFriday;

  return new Date(currentDate.setDate(fridayDate));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (endDate - startDate) / MS_IN_DAY + 1;
}
 ...
 *
 * @example:
 * Date(2024, 0, 3) => 1 - 1,28
 * Date(2024, 0, 31) => 5 - 5,28
 * Date(2024, 1, 23) => 8 - 8,57
 */
function getWeekNumberByDate(date) {
  ...
  for (let i = 1; ; i += 1) {
    const dayOfYear = new Date(Date.UTC(currentYear, 0, i));

    if (dayOfYear.getDay() === 4) {
      firstThursday = dayOfYear;
      break;
    }
  }

  const MS_IN_DAY = 1000 * 60 * 60 * 24;
  const firstWeekOfYear = new Date(firstThursday - MS_IN_DAY * 3);

  return Math.floor((currentUtcDate - firstWeekOfYear) / MS_IN_DAY / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  if (currentDate < 13) {
    currentDate = 13;
    dateObj.setDate(currentDate);
  } else if (currentDate > 13) {
    currentDate = 13;
    currentMonth += 1;
    dateObj.setMonth(currentMonth, currentDate);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [startDate, startMonth, startYear] = start;
  const [endDate, endMonth, endYear] = end;

  const periodStart = new Date(startYear, startMonth - 1, startDate);
  const periodEnd = new Date(endYear, endMonth - 1, endDate);
  ...
  while (periodStart <= periodEnd) {
    if (workingDays < countWorkDays) {
      ...
      schedule.push(`${dateStr}-${monthStr}-${periodStart.getFullYear()}`);
      workingDays += 1;
    } else {
      daysOff += 1;

      if (daysOff === countOffDays) {
        workingDays = 0;
        daysOff = 0;
      }
    }
